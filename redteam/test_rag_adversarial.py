...
def test_rag_inspect_retrieved_chunk_adversarial(payload, expected_rule):
    ...
    assert any(f.rule_id == expected_rule for f in findings), f"Expected {expected_rule} but got {[f.rule_id for f in findings]}"
...
AST_PAYLOADS = [
    ...
    ("store.as_retriever(search_kwargs={'k': sys.maxsize})", ["'k': (lambda x: min(x, 20) if type(x) is int else 10)(sys.maxsize)"]),
    ("store.as_retriever(search_kwargs={'k': get_limit(a, b)})", ["'k': (lambda x: min(x, 20) if type(x) is int else 10)(get_limit(a, b))"]),
    # Filter bypass attempts (Nested dicts)
    ("store.as_retriever(search_kwargs={'filter': {'role': {'$in': ['admin']}}})", ["'filter': {**{'role': {'$in': ['admin']}}, 'tenant_id': tenant_id}"]),
    ("store.as_retriever(search_kwargs={'filter': get_filter()})", ["'filter': {**get_filter(), 'tenant_id': tenant_id}"]),
    
    # --- LangChain kwargs unpacking ---
    ("store.as_retriever(**dynamic_kwargs)", ["**{**dynamic_kwargs, 'search_kwargs': {**dynamic_kwargs.get('search_kwargs', {}), 'filter': {**dynamic_kwargs.get('search_kwargs', {}).get('filter', {}), 'tenant_id': tenant_id}, 'k': (lambda x: min(x, 20) if type(x) is int else 10)(dynamic_kwargs.get('search_kwargs', {}).get('k', 10))}}"]),
    ...
    ("store.similarity_search(query, filter={'role': 'admin'})", ["filter={**{'role': 'admin'}, 'tenant_id': tenant_id}"]),
    ("store.similarity_search(query, filter=get_filter(), k=get_k())", ["filter={**get_filter(), 'tenant_id': tenant_id}", "k=(lambda x: min(x, 20) if type(x) is int else 10)(get_k())"]),
    ...
    ("index.as_query_engine(similarity_top_k=get_limit(a, b))", ["similarity_top_k=(lambda x: min(x, 20) if type(x) is int else 10)(get_limit(a, b))"]),
    ("index.as_query_engine(vector_store_kwargs={'filter': {'$ne': 'user'}})", ["'filter': {**{'$ne': 'user'}, 'tenant_id': tenant_id}"]),
]
...
def test_python_ast_rag_hardening(code, expected_substrings):
    ...
    for substr in expected_substrings:
        assert substr in synthesized, f"Expected '{substr}' in synthesized code:\nOriginal: {context_code}\nSynthesized: {synthesized}"
